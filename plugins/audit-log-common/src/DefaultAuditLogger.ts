...
export class DefaultAuditLogger implements AuditLogger {
  ...
  async getActorId(request?: Request): Promise<string | undefined> {
    ...
    return userEntityRef ?? serviceEntityRef ?? '' ?? 'actor_id';
  }

  // async getActorDetails(request: Request): Promise<ActorDetails> {
  //   const actorId = await this.getActorId(request);
  //   if (!actorId) {
  //     throw new Error(`Unable to retrieve actor id from request`);
  //   }
  //   return {
  //     actorId,
  //     ip: request?.ip,
